...
The portal ships with `react-i18next` (v17) + `i18next-browser-languagedetector` (v8) and is fully trilingual out of the box:

| Locale code | Display name | Market |
|---|---|---|
| `zh-CN` | 中文 | Default (development seed) |
| `es-ES` | Español | Spain primary market |
| `en-ES` | English | English-speakers in Spain market |

### File Layout

```
src/i18n/
├── config.ts             # SUPPORTED_LOCALES, NAMESPACES, FALLBACK_LOCALE
├── index.ts              # i18next init + applyDocumentLocale()
├── useLocale.ts          # zustand bridge
└── locales/
    ├── zh-CN/
    │   ├── common.json   # shared status / buttons / actions
    │   ├── nav.json
    │   ├── policy.json
    │   ├── insurance.json
    │   ├── claims.json
...
Namespaces are loaded eagerly via `import.meta.glob` at build time — there is no runtime fetch and no code-splitting overhead.

### Adding a New Locale

1. Add the code to `SUPPORTED_LOCALES` in `src/i18n/config.ts`.
2. Create `src/i18n/locales/<code>/` and copy all 9 namespace files from `zh-CN/` as a starting point.
3. Translate values, keep the same key tree. Use `{{var}}` for interpolation, `_one` / `_other` suffixes for plurals.
4. Update `LOCALE_LABELS` in `config.ts` with the display name shown in the language switcher.
5. Verify with `npm run build` — TypeScript will catch any missing key in `t()` calls.

### Adding a New String

1. Look up the matching namespace in `locales/zh-CN/`.
2. Add the key under the appropriate section (`buttons.renew`, `renewal.center.kpi.expiring30`, etc.).
3. Replicate the same key in `es-ES/` and `en-ES/`. Missing-translation warnings are emitted in dev (`[i18n] missing key: …`).
4. In the component: `const { t } = useTranslation('namespace')` then `{t('section.key', { var })}`.

### Locale-Aware Patterns

- **Currency**: Use `Intl.NumberFormat(locale, { style: 'currency', currency: 'EUR' })` (or call `formatCurrency` in `src/utils/locale.ts`).
- **Date**: Use `Intl.DateTimeFormat(locale)` (or `formatDate` helper). Avoid hardcoded `YYYY-MM-DD` in JSX.
- **Phone validation**: `isValidPhone(value, locale)` in `src/utils/locale.ts` checks `zh-CN` (11-digit) / `es-ES` (9-digit or +34) / fallback E.164.
- **NIF / IBAN**: `validateNif` and `validateIban` for Spain-specific inputs.
- **Status badges**: All shared via `common.status.*` (active, expired, eligible, paid, reviewing, etc.). Do not invent new status keys per page.

### Direction & `lang` Switching

`applyDocumentLocale(lng)` in `src/i18n/index.ts` writes both `html.lang` and `html.dir` on every language change. Right now all three supported locales are LTR; Arabic / Hebrew would be added by extending `RTL_LANGS` in that file. CSS hooks in `src/styles/global.css` provide `html[dir="rtl"]` mirror rules and `.lang-zh / .lang-es / .lang-en` body classes for typography tweaks.

### UX Recommendations When Localizing

- **Spanish broker terminology** (audit before translating):
  - 保单 → `póliza`
  - 投保 → `contratar` / `suscribir`
  - 续保 → `renovación`
  - 理赔 → `siniestro` (rate → `siniestralidad`)
  - 雇主责任险 → `Responsabilidad Civil Patronal` (or `Accidentes de Convenio`)
  - 询价单 → `cuestionario` / `solicitud de cotización`
- **Date in Spanish**: `Intl.DateTimeFormat('es-ES')` yields `19/06/2026` — preferred for legal/broker documents over `Jun 19, 2026`.
- **Currency in Spanish**: `1.234,56 €` (period as thousands separator, comma as decimal) is the convention; `Intl.NumberFormat` handles this automatically.
- **NIF / NIE / IBAN** validation should run on form submit, not on every keystroke, to avoid scolding users mid-input.
